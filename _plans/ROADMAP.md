...
- **Goal**: A consumer can bind and validate the `Cloudstrap:` configuration section into a typed `CloudstrapConfiguration` model.
...
- **Depends on**: 0
- **Migration decisions**: section `Nihdi:` → `Cloudstrap:`; **cut the inverted Dashboard.Contracts reference** (dashboard settings move to deliverable 19); per-feature settings move out (`Settings\NServiceBus\` → 14, `Settings\Hangfire\` → 16, `Settings\Security\` → 9/10, `Settings\Swagger|Scalar\` → 5); delete `Settings\Logging\DynatraceConfiguration.cs` and Bridge settings; `Nihdi.Core.Functional` edge → LanguageExt.Core (settle exact type mapping — first consuming package).
...
- **Definition of done**: build/tests/format green; XML docs on all public API; package metadata complete; zero `Nihdi`/`Riziv` identifiers; validation errors raised via `ConfigurationValidationException`-style guard behavior; no reference to any Dashboard type.
- **Risks**: ⚠️ Public API surface for every later package — shape mistakes propagate; ⚠️ LanguageExt.Core type-mapping decision lands here first; ⚠️ deciding which settings stay in Core vs move out changes every later deliverable.
- **Status**: ⬜
