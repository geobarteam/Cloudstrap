...
| 5 | WebApi bootstrap ← **next** | Cloudstrap.WebApi | 4 ✅ | ⬜ | — |
...
`Cloudstrap.WasmTestProject.E2E.Tests`, 3 new E2E tests). **#4 → ✅.** Tally: **6 ✅ (#0, #1,
#2, #3, #4, #25) · 0 in flight (🔨/📝) · 20 remaining (#5–#24)**. Full `src/**/*.csproj`
sweep confirms exactly four shipped packages (Core, Observability, Observability.AzureMonitor,
Extensions) + four unit-test projects + the five WASM-SUT/E2E projects — no orphan or
undeclared project. **Nothing is in flight**: #5 (`Cloudstrap.WebApi`) is next and enters
technical analysis; its only dependency (4) is ✅.
...
### 5. WebApi bootstrap ← **next (enters technical analysis)**
...
- **Source material** *(file inventory verified against the reference repo 2026-08-02)*: `Nihdi.Core.Configuration.WebApi\` —
  ...
  - `Correlation\CorrelationMiddleware.cs` — ⚠️ **likely superseded**: #2 already shipped `Cloudstrap.Observability.Correlation` (accessor/source/middleware/attributes). Analyst must diff and either drop it or state exactly what WebApi adds.
  ...
  - Source `PackageReference` set to replace: `Asp.Versioning.Mvc` + `.Mvc.ApiExplorer` 10.0.0 (keep), `NSwag.AspNetCore` 14.7.1 (⚠️ decide — see risks), `Nihdi.AspNetCore.Authentication.JwtBearer` 5.2.5 (→ stock), `Nihdi.Core.Health` 1.0.24 (→ already replaced by #4's `MapCloudstrapHealthChecks`).
...
- **Migration decisions**: `Nihdi.AspNetCore.Authentication.JwtBearer` → stock JWT bearer with hardened defaults (audience validation on, reduced clock skew, HTTPS metadata outside Development); `Nihdi.Core.Health` → **do not re-implement** — reuse #4's `MapCloudstrapHealthChecks` and the `CloudstrapHealthCheckTags` `"live"`/`"ready"` contract from #2; correlation comes from #2, not from a second middleware.
- **De-NIHDI items**: API naming `AddNihdiX` → `AddCloudstrapX`; the `ForDevTst` environment taxonomy → an explicit, overridable option; neutral fixtures (`example.com`, `contoso`) in Swagger/Scalar samples and OAuth scope defaults; no internal IdP/authority URLs in defaults.
...
- **Risks**: ⚠️ **Auth code** (risk area — human review): `AddCloudstrapJwtBearer` is the first shipped authentication surface. ⚠️ **OpenAPI stack is a one-way door**: .NET 10 ships built-in `Microsoft.AspNetCore.OpenApi`, so "keep NSwag" vs "stock OpenAPI + `Scalar.AspNetCore` UI" is a genuine analyst decision, not a mechanical port — it determines the public options shape (spec's Package Map says "NSwag/Scalar"). ⚠️ New deps `Asp.Versioning.Mvc`/`.ApiExplorer` (MIT), `Scalar.AspNetCore` (MIT), possibly `NSwag.AspNetCore` (MIT) — each needs a CPM pin. ⚠️ **Aspire overlap**: health-check endpoints — register additively through the stock `IHealthChecksBuilder` and do not re-map what #4 already maps (AC-ASP3 posture); the OpenAPI/versioning surface itself does not overlap ServiceDefaults. ⚠️ Sets the middleware-pipeline pattern that #6 (Mvc) and #7 (Worker) inherit — a shape mistake propagates twice.
- **Status**: ⬜ — next; technical analysis not yet started (`_specs/5-WebApiBootstrap.md` to be written).
