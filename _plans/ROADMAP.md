...
| 2 | Observability base ← **next** | Cloudstrap.Observability | 1 ✅ | ⬜ | — |
...
prescribed). `_plans/1-CoreSettingsModel.md` is 19/19 boxes checked (5 steps, 3 gates
approved) → #1 is ✅. `_plans/0-RepoScaffolding.md` has all 5 steps done and its Slice-1
gate approved, but **6 boxes of its final gate are still open** → #0 stays 🔨. Those 6 are
operational/GitHub-side (prefix reservation, GitVersion probe review, PR + `dev` CI runs,
`cleanup-previews.yml` dispatch, optional `v0.1.0` tag, workflow code review): they gate the
**first publish**, not local development or the next port — deliverables 2+ may proceed.
Everything 3–24 is honestly ⬜.
...
- **Status**: 🔨 — **code complete, gate open**. All 5 plan steps are done and the Slice-1 gate is approved (2026-07-25); the local rails (build/test/format/CPM/analyzers) and all three workflows exist and were proven locally, which is why deliverable 1 could build on them. Six boxes remain unchecked in the plan's final 🛑 gate: nuget.org `Cloudstrap.` prefix reservation · GitVersion probe review (AC-R9) · PR-run of `ci.yml` (AC-R4) · `dev`-branch preview-publish no-op run (AC-R5) · `cleanup-previews.yml` manual dispatch · optional `v0.1.0` tag (AC-R6) · workflow code review + De-NIHDI sweep (AC-R8). **These gate the first *publish*, not local development** — they must be closed before any `Cloudstrap.*` package is pushed to a feed, but they do not block deliverables 2+. The user reported on 2026-07-26 that two of the items are done without identifying which, so all six stay unchecked in `_plans/0-RepoScaffolding.md` until its owner closes them.
...
- **Status**: ✅ — done 2026-07-26 (plan's final 🛑 gate checked; definition of done holds). Not yet *published*: the first feed push is gated by deliverable 0's open operational items.
...
- **Status**: ⬜ — **next deliverable**. Dependencies: 1 ✅ (0 is 🔨 but its open items are publish-only). Next step: technical-analyst writes `_specs/2-ObservabilityBase.md`.
...
| 2026-07-26 | **Deliverable #0 status corrected 📝 → 🔨** (not ✅). All 5 scaffolding steps are done and the Slice-1 gate is approved, but six boxes of the final 🛑 gate are still open: nuget.org prefix reservation · GitVersion probe review · PR `ci.yml` run · `dev` preview-publish no-op run · `cleanup-previews.yml` dispatch · optional `v0.1.0` tag · workflow code review. Recorded explicitly that these gate the **first publish**, not local development — deliverables 2+ are unblocked. The user reported two of them done on 2026-07-26 without identifying which, so none are checked. | Roadmap rule: ✅ only when the plan's final gate is checked `[x]`. Honest bookkeeping beats a green square; the distinction (publish-blocking vs work-blocking) is what keeps the extraction moving without losing the release prerequisite. |
