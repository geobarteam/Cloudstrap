...
- [x] Behavioral verification (AC-DR1): `dotnet build src/Cloudstrap.sln` + all 9 unit exes + `Cloudstrap.Demo.E2E.Tests.exe` (full run — pre-existing behavioral assertions unchanged, plus the new `ApiHostTests`, the cross-process `UserCall` proof, and `BlazorServerTests`) + `dotnet format --verify-no-changes` — all green, output presented (576 unit + 48 E2E).
- [x] AC-DR2: CI green on the branch — glob discovers every test project incl. the relocated E2E suite; `playwright.ps1` path resolves (user pushes / reviews the run).
- [x] AC-DR4 (user-run, manual): the VS Code compound launch boots all demo hosts in dependency order (IdP first); browsing the Bff's `/doctors` auto-triggers login at the shared IdP; stopping the session stops all hosts.
- [x] AC-DR5 feature walk confirmed (E2E + optionally manual): OIDC login (BlazorWasm + BlazorServer), client-credentials M2M (Bff `SelfApi`), JWT cross-process hop (Bff → Api, `host: demo-api`), observability Console + Otlp + AzureMonitor all exercised.
- [x] AC-DR11: the spec's breakage-checklist table walked item-by-item (sln incl. duplicate-nesting removal · `src/Test/Directory.Build.props` + `src/demo/Directory.Build.props` · ci.yml · `SutProcess.cs` · `PageTestBase.cs` · `.vscode` · `Directory.Packages.props` comments · READMEs · AC-D11 reconciliation · standing-rule doc set) — every row closed; the Step 7 grep-sweep hit list reviewed (historical docs only).
- [x] AC-DR8 + AC-DR12 doc review: per-app feature matrices accurate against `Program.cs`/config/E2E; the canonical standing-rule wording approved (or amended) across CLAUDE.md / planner.md / project-manager.md / tests.md / plan-template.md; the `_specs/WasmTestProjectDemoCompletion.md` reconciliation note approved.
- [x] AC-DR9 + AC-DR13 + AC-DR14: full-diff sweep — zero secrets (placeholder-only, visibly fake), zero `Nihdi`/`NIHDI`/`Riziv`, zero `Aspire.*`, zero real PII; demo apps use ProjectReferences to shipped packages only, no #12/#13 helper code; nothing under `src/demo` packs in Release.
- [x] AC-DR10: `git log --follow` spot checks across move + rename commits confirmed.
- [x] User approved — deliverable #27 done (2026-08-20; project-manager then flips §27 ✅ and applies the roadmap-preamble standing-rule update; the D-E authorization-demo follow-up plan may be scheduled).
