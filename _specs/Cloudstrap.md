...
| Claim check / DataBus (Azure Blob) | `Cloudstrap.Messaging.AzureBlob` is a thin seam over **Wolverine's native claim check** and its Azure Blob store (`WolverineFx.AzureBlobStorage`): payloads over a size threshold (default 200 KiB) are stored to Blob, a reference travels in the message, the handler reads it transparently. No bespoke offload middleware — Wolverine ships the feature since 6.2x, Cloudstrap binds it to its conventions (dedicated `{system}-claimcheck` container, `DefaultAzureCredential` via the registered blob client, fail-fast, dead-letter posture). *(amended 2026-09-21 — the original "Wolverine has no built-in claim check" premise was stale; see `_specs/15-BlobClaimCheck.md` Decision Log, DL-5)* |
