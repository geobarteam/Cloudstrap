...
>
> **Post-approval amendments** (recorded in the Decision Log; the code was already shipped, so each is implemented by a later deliverable): 2026-07-25 — `IsRunningInAks()` verdict Redesign → **Drop**. 2026-07-27 — the `Mode = Otlp` ⇒ `Endpoint` required rule is **relaxed** to accept `OTEL_EXPORTER_OTLP_ENDPOINT` (AC-C6 re-worded; implementation scheduled inside deliverable #2's plan).
>
...
| AC-C6 *(amended 2026-07-27 — see Decision Log)* | `Cloudstrap:OpenTelemetry:Mode = Otlp` with (a) no `Endpoint` **and** no `OTEL_EXPORTER_OTLP_ENDPOINT` in configuration, (b) no `Endpoint` **but** `OTEL_EXPORTER_OTLP_ENDPOINT` present, (c) a non-HTTP(S) `Endpoint`, (d) a valid `Endpoint` | Options validation runs | (a) fails with a message naming `Endpoint` **and** the `OTEL_EXPORTER_OTLP_ENDPOINT` alternative; (b) **passes** — the endpoint is resolved by the OpenTelemetry SDK, and deliverable 2 leaves the exporter endpoint unset; (c) fails naming `Endpoint`; (d) passes. The rule is relaxed, not removed: `Mode = Otlp` with neither source of an endpoint still fails. `Mode = AzureMonitor` requires nothing extra at the Core level (exporter settings are deliverable 3's). |
...
│     Endpoint           : Uri?    — http/https; required when Mode = Otlp unless
│                                    OTEL_EXPORTER_OTLP_ENDPOINT is in configuration
│                                    (amended 2026-07-27)
...
Validation mechanics (behavioral requirement; exact mechanism is the planner's choice): DataAnnotations for simple required/range rules; conditional and cross-property rules (`Otlp` ⇒ `Endpoint` **unless `OTEL_EXPORTER_OTLP_ENDPOINT` is present in configuration** — *amended 2026-07-27*, which means this one rule needs `IConfiguration` in the validator; `File.Enabled` ⇒ `Path`; `BaseAddress` absolute) via `IValidateOptions<T>` — the framework's source-generated `[OptionsValidator]` with `[ValidateObjectMembers]` is the preferred implementation because it removes the hand-rolled nested-object cascade (finding 3) and is reflection-free. Both the DI path and the eager `GetCloudstrapOptions` path must apply the *same* rules (single validator implementation, two entry points).
...
12. **The `Otlp` ⇒ `Endpoint` rule accepts the ecosystem's standard variable** *(amendment 2026-07-27)* — `Endpoint` is required when `Mode = Otlp` only while `OTEL_EXPORTER_OTLP_ENDPOINT` is absent from configuration. With the standard variable present, validation passes and the OpenTelemetry SDK resolves the endpoint (deliverable 2 then configures none). Explicit Cloudstrap configuration still wins; configuring neither still fails startup. The source library was stricter still — it threw whenever `BaseUri` was empty in `Otlp` mode.
...
## Decision Log (gate answers 2026-07-25 + post-approval amendments — zero Open Questions remain; spec is planner-ready)
...
| Amendment (post-approval, 2026-07-27) — the `Mode = Otlp` ⇒ `Endpoint` required rule | **Relaxed, not removed.** `Endpoint` stays required when `Mode = Otlp` **unless `OTEL_EXPORTER_OTLP_ENDPOINT` is present in configuration**; with neither, validation still fails. Raised as OQ-2 of `_specs/2-ObservabilityBase.md` and answered by the user on 2026-07-27 (option B). Reason: the OpenTelemetry SDK reads `OTEL_EXPORTER_OTLP_ENDPOINT`/`_HEADERS`/`_PROTOCOL` itself, and Aspire and the OpenTelemetry Operator inject that variable into every workload they launch — the shipped rule made the ecosystem's default deployment convention fail startup with what looks like a Cloudstrap bug, contradicting the founding spec's Aspire posture §3 ("support … well-known environment variables where sensible"). Consequences recorded above: **AC-C6** re-worded (four cases), the `Settings\OpenTelemetryConfiguration.cs` row annotated, the validation-mechanics paragraph noting that this one rule needs `IConfiguration` in the validator, and Deliberate Behavior Change 12 added. **Implementation lands in `src/Cloudstrap.Core/OpenTelemetryOptionsValidator.cs` and its tests, scheduled inside deliverable #2's plan** — deliverable #1 itself stays closed (its plan is complete and its final gate checked); this is a spec amendment carried out by the next deliverable, not a re-opening of #1. Deliverable 2 consumes the relaxed rule in AC-B13. |
