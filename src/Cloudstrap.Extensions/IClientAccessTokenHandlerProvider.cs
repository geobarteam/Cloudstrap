namespace Cloudstrap.Extensions
{
    ...
    /// <summary>
    /// Supplies the message handler that attaches an application (client credentials) access token to a
    /// Cloudstrap typed HTTP client. This package declares the seam and never implements it: the
    /// implementation ships with <c>Cloudstrap.Authentication.ClientCredentials</c>, so nothing here depends
    /// on an authentication stack.
    /// </summary>
    /// <remarks>
    /// <para>
    /// A client opts in through its configuration — <c>Cloudstrap:HttpClients:{name}:AddClientAccessToken</c>.
    ...
    /// an application token and no provider is registered, client creation fails with a message naming the
    /// flag and the package that supplies the implementation — an unauthenticated request is never sent in
    /// its place.
    /// </para>
    /// <para>
    ...
    /// </para>
    /// </remarks>
    public interface IClientAccessTokenHandlerProvider
    {
        /// <summary>
